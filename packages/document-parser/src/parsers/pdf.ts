...
export async function parsePdf(filePath: string, _outputDir: string, limits: DocumentParseLimits): Promise<ParserResult> {
  ...
  try {
    ...
    return { text: "", warnings: [`PDF 文件可能损坏、加密或格式异常，无法读取内容。${String((error as Error).message ?? error).slice(0, 120)}`], limitsHit: ["pdfParseFailed"], unsupported: true, unsupportedImages: true };
  } finally {

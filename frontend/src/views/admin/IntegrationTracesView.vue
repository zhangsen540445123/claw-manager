...
async function open(row:Summary){
  drawer.value=true;detailLoading.value=true;selected.value=row;timeline.value=[];related.value=[];artifact.value={};
  try {const result=await api<Detail>(`/api/admin/integration-traces/${encodeURIComponent(row.traceId)}`);timeline.value=result.timeline;related.value=result.relatedRequestIds;artifact.value=result.artifact??{};}

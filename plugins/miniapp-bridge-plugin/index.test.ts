...
describe("miniapp bridge", () => {
  ...
  it("publishes a generated image by opaque server-side image id", async () => {
    ...
    const result = await callArtifactBridge(
      { operation: "publish_image", generatedImageId: "img_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", title: "海报" },
      { requesterSenderId: "wechat-user-1" }, env, fetcher as typeof fetch,
    );
  ...
  it("reports a sanitized artifact tool error", async () => {
    ...
    await expect(callArtifactBridge(
      { operation: "publish_image", generatedImageId: "img_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" },
      { requesterSenderId: "wechat-user-1" }, env, fetcher as typeof fetch,
    )).rejects.toThrow("miniapp artifact request failed (404)");

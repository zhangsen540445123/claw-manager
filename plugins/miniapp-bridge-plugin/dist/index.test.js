...
describe("miniapp bridge", () => {
    ...
    it("forwards only sender scope and approved business parameters", async () => {
        const fetcher = vi.fn(async (_url, init) => new Response(JSON.stringify({ result: { code: 200 } }), { status: 200 }));
        await callMiniappBridge({ actionKey: "daily_checklist", parameters: { date: "2026-07-12" } }, { requesterSenderId: "wechat-user-1" }, env, fetcher);
    ...
    it("rejects an unknown action before calling the network", async () => {
        const fetcher = vi.fn();
        await expect(callMiniappBridge({ actionKey: "arbitrary_http", parameters: {} }, { requesterSenderId: "sender" }, env, fetcher))
            .rejects.toThrow("unsupported miniapp actionKey");
        expect(fetcher).not.toHaveBeenCalled();
    });
    it("rejects missing sender identity without fallback", async () => {
        await expect(callMiniappBridge({ actionKey: "goal_list" }, {}, env, vi.fn()))

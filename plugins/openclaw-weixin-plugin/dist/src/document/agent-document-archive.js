...
function toRelative(workspace, target) {
    return path.relative(workspace, target).split(path.sep).join("/");
}
export async function archiveAndParseWeixinDocument(input) {
    const safeName = sanitizeFilename(input.filename);
    const messageDir = path.join(input.workspace, ".openclaw-inbox", "weixin", input.day ?? todayCompact(), sanitizeSegment(input.messageSid));
    ...
    const normalizedParsed = warnings.length === parsed.warnings.length
        ? parsed
        : { ...parsed, warnings };
...
export function findAgentWorkspaceFromConfig(cfg, agentId) {
    if (!agentId)
        return undefined;
    const agents = cfg.agents && typeof cfg.agents === "object" ? cfg.agents : undefined;
    const list = Array.isArray(agents?.list) ? agents.list : [];
    const agent = list.find((entry) => Boolean(entry) && typeof entry === "object" && entry.id === agentId);
    return typeof agent?.workspace === "string" && agent.workspace.trim() ? agent.workspace : undefined;
}
//# sourceMappingURL=agent-document-archive.js.map
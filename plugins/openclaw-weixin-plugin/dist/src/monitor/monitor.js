...
export async function monitorWeixinProvider(opts) {
    ...
    while (!abortSignal?.aborted) {
        ...
        try {
            ...
            errLog(`weixin getUpdates network error (${consecutiveFailures}/${MAX_CONSECUTIVE_FAILURES}): ${String(err)} type=${classified.type} description=${classified.description}${classified.code ? ` code=${classified.code}` : ""}`);
            aLog.error(`getUpdates network error: ${String(err)}, type=${classified.type} code=${classified.code ?? "none"}, stack=${err.stack}`);
        ...
        if (isApiError) {
            ...
            if (isStaleToken) {
                ...
                aLog.error(`getUpdates: token for ${redactIdentity(accountId)} is stale, pausing all requests for ${Math.ceil(pauseMs / 60_000)} min`);
                consecutiveFailures = 0;
        ...
        for (const full of list) {
            aLog.info(`inbound message: from=${redactIdentity(full.from_user_id)} types=${full.item_list?.map((i) => i.type).join(",") ?? "none"}`);
            const now = Date.now();
...
async function processMessageWithIsolation(full, opts, configManager, errLog, aLog) {
    let lastError;
    for (let attempt = 1; attempt <= MAX_MESSAGE_PROCESSING_ATTEMPTS; attempt += 1) {
        try {
            ...
            if (errorCode === "WECHAT_AGENT_NOT_READY") {
                errLog(`weixin agent not ready (${attempt}/${MAX_MESSAGE_PROCESSING_ATTEMPTS}): account=${redactIdentity(opts.accountId)} peer=${redactIdentity(full.from_user_id)} code=${errorCode}`);
                aLog.warn(`agent not ready: attempt=${attempt} error=${redactedError}`);
            }
            else {
                errLog(`weixin message routing error (${attempt}/${MAX_MESSAGE_PROCESSING_ATTEMPTS}): account=${redactIdentity(opts.accountId)} peer=${redactIdentity(full.from_user_id)} code=${errorCode}`);
    ...
    try {
        ...
        errLog(`weixin message quarantined: account=${redactIdentity(opts.accountId)} peer=${redactIdentity(full.from_user_id)} code=${errorCode}`);
...
function messageProcessingErrorCode(error) {
...
async function waitAfterGetUpdatesFailure(consecutiveFailures, errLog, aLog, abortSignal) {
    if (consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {

...
export type WeixinMsgContext = {
  ...
  ChatType: "direct";
  /** Canonical OpenClaw sender identity field used by channel runtime internals. */
  SenderId?: string;
  /** Set by monitor after resolveAgentRoute so dispatchReplyFromConfig uses the correct session. */
  SessionKey?: string;
  /** Stable Weixin sender identity passed to OpenClaw runtime context. */
  senderId?: string;
  /** Stable Weixin sender identity passed to trusted inbound tool context. */
...
export function weixinMessageToMsgContext(
  ...
  const ctx: WeixinMsgContext = {
    ...
    senderId: from_user_id,
    requesterSenderId: from_user_id,
  };

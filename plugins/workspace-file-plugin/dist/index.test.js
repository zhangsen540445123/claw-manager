...
describe("native read guard", () => {
    ...
    it("blocks dynamic WeChat agents from reading outside allowed roots", async () => {
        ...
        const result = await guardNativeRead({ agents: { defaults: { workspace: path.join(home, "shared") }, list: [{ id: "wechat_user", workspace: agentWorkspace }] } }, { toolName: "read", params: { path: path.join(outside, "secret.txt") } }, { agentId: "wechat_user" });

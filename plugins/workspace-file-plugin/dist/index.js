...
class WorkspaceFileError extends Error {
    workspaceFileError = true;
}
function fail(message) {
    throw new WorkspaceFileError(message);
}
function safeFsMessage(error) {
    switch (error.code) {
...
async function existingAncestor(candidate) {
    ...
    while (true) {
        try {
            ...
            if (parent === current)
                throw new Error("workspace file path cannot be resolved");
            current = parent;
        }
    }
}
async function resolveWorkspacePath(workspaceDir, inputPath) {
    ...
    if (!rootStat.isDirectory())
        return fail("workspace directory is invalid");
    const absolute = path.resolve(workspaceDir, relative);
    const targetRealPath = await realpath(absolute).catch(async (error) => {
        if (error.code !== "ENOENT")
            throw error;
...
async function currentSha256(filePath) {
    try {
...
async function executeList(resolved) {
...
export async function executeWorkspaceFile(workspaceDir, input) {
    try {
